import './BlogDetailPage.css'
import {Link, useNavigate, useParams} from "react-router-dom";
import NavBar from "../../components/Navigatie/NavBar.jsx";
import {formatDate} from "../../helpers/formatDate.js"
import {useEffect, useState} from "react";
import axios from "axios";


function BlogDetailPage() {
    const {id} = useParams();
    const [post, setPost] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [deletePost, setDeletePost] = useState(null);
    const navigate = useNavigate();


    async function handleDelete() {
        try {
            const response = await axios.delete(
                `https://novi-backend-api-wgsgz.ondigitalocean.app/api/blogposts/${id}`,
                {
                    headers: {
                        "Content-Type": "application/json",
                        "novi-education-project-id": "d6200c4d-2a0a-435d-aba6-6171c6a7296e",
                    },
                }
            );
            console.log("Post verwijderd:", response.data);
            setDeletePost("De blogpost is succesvol verwijderd");

            setTimeout(() => {
                navigate("/blog-overview")
            }, 3000)

        } catch (error) {
            console.error("Fout bij verwijderen:", error);
            alert("Er is iets misgegaan bij het verwijderen van de post.");
        }
    }

    useEffect(() => {
        const fetchPost = async () => {
            try {
                // Haal alle posts op van de backend
                const response = await axios.get(
                    "https://novi-backend-api-wgsgz.ondigitalocean.app/api/blogposts",
                    {
                        headers: {
                            "Content-Type": "application/json",
                            "novi-education-project-id": "d6200c4d-2a0a-435d-aba6-6171c6a7296e",
                        },
                    }
                );

                const data = response.data;
                console.log("Data opgehaald:", data);

                // Zoek de juiste post op basis van id uit de URL
                const foundPost = data.find((item) => String(item.id) === String(id));

                if (!foundPost) {
                    throw new Error("De gevraagde post kon niet worden gevonden.");
                }

                console.log("Gevonden post:", foundPost);
                setPost(foundPost);
            } catch (err) {
                console.error(err);
                setError("Er is een fout opgetreden bij het ophalen van de post.");
            } finally {
                setLoading(false);
            }
        };

        fetchPost();
    }, [id]);


    if (loading) {
        return (
            <>
                <NavBar/>
                <p>Bezig met laden...</p>
            </>
        );
    }

    if (error) {
        return (
            <>
                <NavBar/>
                <p style={{color: "red"}}>{error}</p>
            </>
        );
    }

    if (!post) {
        return (
            <>
                <NavBar/>
                <p>Geen post gevonden.</p>
            </>
        );
    }

    return (
        <>
            <NavBar/>
            <section className="blog-detail">
                <article className="blog-post">
                    <div className="blog-post-content">
                        <strong><h2>{post.title}</h2></strong>
                        <strong>{post.subtitle}</strong>
                        <p>Geschreven door {post.author} op {formatDate(post.created)}</p>
                        <p>{post.content}</p>
                        <p>{post.comments} reacties - {post.shares} gedeeld</p>
                    </div>
                    <Link to="/blog-overview">← Terug naar overzicht</Link>
                    <button className="delete-button" type="button" onClick={handleDelete}>Blog Verwijderen</button>
                </article>
                {deletePost ? <p>Deze blogpost word verwijderd</p> : ""}
            </section>

        </>
    )
}

export default BlogDetailPage;